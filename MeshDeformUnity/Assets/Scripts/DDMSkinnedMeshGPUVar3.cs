...
//[ExecuteInEditMode]
public class DDMSkinnedMeshGPUVar3 : DDMSkinnedMeshGPUBase
{
	...
	void Start()
	{
		...
		if (computeShader && ductTapedShader)
		{
			...
			lastcolsCB = new ComputeBuffer(vCount * maxOmegaCount, (4 * sizeof(float) + sizeof(int)));
			...
			computeShader.SetBuffer(deformKernel, "Lastcols", lastcolsCB);
		...
		DDMUtilsGPU.ComputeLastcolsCBPsCBFromOmegasCB(ref lastcolsCB, ref psCB, precomputeShader, omegasCB, vCount, bCount);
